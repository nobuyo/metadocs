...
export const getAllCategories = () => {
  return fs.readdirSync('docs', { withFileTypes: true })
    ...
    .map((dir) => {
      const raw = fs.readFileSync(path.resolve('docs', dir.name, '_category_.json'), 'utf-8');
      const category = JSON.parse(raw) as Category;
      category.slug = dir.name.replace('/', '');

      const articles = glob.sync(path.resolve('docs', dir.name, '*.md')).map((fileName: string): Article => {
        const raw = fs.readFileSync(fileName, 'utf-8');
        const data = matter(raw).data;

        const slugFromFileName = fileName.split('/').reverse()[0].replace('.md', '')
